    ...
    await Promise.all(
      trip.participants.map(async (participant) => {
        ...
        const message = await mail.sendMail({
          from: {
            name: 'Plann.er | Team',
            address: 'admin@plann.er'
          },
          to: participant.email,
          subject: `Confirme sua presenca na viagem para ${trip.destination} em ${formattedStartDate}.`,
          html: `
                <div style="font-family: sans-serif; font-size: 16px; line-height: 1.6;">
                    <p>Você foi convidado(a) para participar de uma viagem viagem para <strong>${trip.destination}</strong> nas datas de <strong>${formattedStartDate}</strong> até <strong>${formattedEndDate}</strong>.</p>
                    <p></p>
